...
let Graph = React.createClass({
  componentDidMount: function () {
    let initialPosition = this.props.annotations[this.props.position].position
    ...
    d3.csv('./leso-1033-counts.csv', convert, function(error, data) {
      ...
      svg.selectAll("dot")
        ...
        .enter().append("circle")
          ...
          d3.select('circle[index="' + initialPosition + '"]')
            .attr('class', 'selected-point')
            .attr('r', 4.5)

          svg.insert('circle', 'circle[index="' + initialPosition + '"]')
          .attr('class', 'selected-outline')
          .attr('r', 9.5)
          .attr('cx', d3.select('circle[index="' + initialPosition + '"]').attr('cx'))
          .attr('cy', d3.select('circle[index="' + initialPosition + '"]').attr('cy'))
          .attr('id', 'ring');
    });
  ...
  componentWillReceiveProps: function (nextProps) {
    let selected = d3.select('circle[index="' + this.props.annotations[nextProps.position].position + '"]')
    let previous = d3.select('circle[index="' + this.props.annotations[this.props.position].position + '"]')
    ...
    g.insert('circle', 'circle[index="' + this.props.annotations[nextProps.position].position + '"]')
      .attr('class', 'selected-outline')
      .attr('r', 0)
      .attr('cx', selected.attr('cx'))
      .attr('cy', selected.attr('cy'))
